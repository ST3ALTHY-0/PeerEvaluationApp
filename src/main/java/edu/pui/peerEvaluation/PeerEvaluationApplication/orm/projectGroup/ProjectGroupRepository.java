...
public interface ProjectGroupRepository extends JpaRepository<ProjectGroup, Integer> {
    ...
    @Query("SELECT COUNT(s) FROM ProjectGroup pg JOIN pg.students s WHERE pg.groupId = :projectGroupId")
    Integer countStudentsInProjectGroup(@Param("projectGroupId") Integer projectGroupId);
