...
public class AssignRolesService{
    ...
    //the class should get data (or just look at data already gotten) from brightSpace api to see class list of user and what role that person has for each active class
    //and based on that data apply correct authorities here

    //also if we need to do any logic about restricting access to people within CIT we can do so
    ...
    public Collection<? extends GrantedAuthority> getAuthorities(OAuth2User principal) {
        ...
        if (email != null && email.endsWith("@gmail.com")) {
            ...
            return Arrays.asList(new SimpleGrantedAuthority(Roles.STUDENT), new SimpleGrantedAuthority(Roles.INSTRUCTOR));
        }
    }
}